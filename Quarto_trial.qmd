...
author: 
  ...
  - name: "additional author name here"
date: "`r Sys.Date()`"
format: 
  html:
...
## There are three components to this document (when viewing the code)
...
3)  Create Nearshore & Offshore species dataframes (or load if they already exist)
...
6)  Transform data to Phyloseq Object

```{r create-phyloseq-object}

# AM - did not delete original code - review with Collin to get OK to replace with this method. Can we find a more reproducible way of selecting columns? 

names(esv_clean_data_df)

#create an OTU/ESV count table

# AM - converted this to a single piped operation for clarity
# esv_read_count <- select(esv_clean_data_df, -c(1))
...
esv_read_count_m <- esv_clean_data_df %>%
  ...
  select(-c(2:19)) %>% # remove columns 2-19
...
# AM - what is the point of printing out sums of columns & rows?
# rowSums(esv_read_count)
# colSums(esv_read_count)

ESV <- phyloseq::otu_table(esv_read_count_m, taxa_are_rows = TRUE)

# AM - do we need to write/save phyloseq object? Can generate with this code. 
...
# esv_read_tax <- select(esv_clean_data_df, -c(1,3))
...
7)  Explore Components of Phyloseq Object (if needed)
...
8)  Subset Phyloseq Samples - Keep Environmental Samples Only

```{r phyloseq-env-samples_only}
physeq_env <- subset_samples(physeq1, Control_or_Envtl =="Envtl")
# should not replace object with same name - harder to troubleshoot - new versions need new names
```

9)  Barplot - Families - Environmental Samples (excluding Controls)
...
10) Subset Phyloseq Environmental Samples - ESVs with marine fish and mammal annotations

```{r create-env-marine-fish-mammal}
#Select for only Classes: Actinopteri and Mammalia, which will remove Classes: Aves and Amphibia, as well as the unannotated ESVs 
#Within Mammalia remove non-marine mammals in  Family: Bovidae, Suidae, Canidae, Felindae, Hominidae and Order: Primates

# AM - These subsequent lines of code keep replacing the same data object, they do not layer ontop of previous results. Is this the intention? Or was this a discovery of what is in the data and then builds new subset based on the results (which is not useful for a new dataset) We can encode the final subset to be based on flexible results
physeq_env
physeq1_mar <- subset_taxa(physeq_env, Class %in% c("Actinopteri", "Mammalia"))
physeq1_mar
physeq1_mar <- subset_taxa(physeq_env, !(Order %in% c("Primates")))
physeq1_mar
physeq1_mar <- subset_taxa(physeq_env, !(Family %in% c("Bovidae", "Canidae", "Felindae", "Hominidae", "Suidae")))
physeq1_mar

```

11) Environmental Marine Samples With More Than 10,000 Sequences
...
12) Standardize number of reads to the median sequencing depth

```{r std-reads-meadian-seq-depth}
# AM - what is this doing?
total <- median(sample_sums(physeq1_mar_pruned))
standf <- function(x, t=total) round(t *(x / sum(x)))
physeq2 <- transform_sample_counts(physeq = physeq1_mar_pruned, 
                                   fun = standf)
...
13) Barplots of normalized environmental samples and marine ESVs
...
14) Heatmaps of normalized environmental samples and marine ESVs

```{r figure-heatmap-normal-env-marine-phylum}
plot_heatmap(physeq2, taxa.label="Phylum")
```

15) Alternative barplot formats
