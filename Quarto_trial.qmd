...
We used the `renv` package to manage & record package versions. This may 
...
# path to folder that contains all data
dir_data <- file.path("./data/")
# path to folder containing outputs from code
dir_products <- file.path("./products/")
# path to code scripts
...
```{r load-data}
esv_data_all <- read_csv(file.path(dir_data, "JVB1710-MiFishU-read-data.csv"))
# ESV - exact sequence variant

# to see a snipit of the data run this in the console 
#glimpse(esv_data_all)
```

4) clean and save analysis data

```{r clean-data}
#Remove preliminary run (i.e., remove duplicated samples "S046...1" that have newer ".2" version) & cleanup columns

data <- esv_data_all %>% 
  select(-c(S046420.1, S046419.1, S046417.1, S046416.1, S046415.1, S046414.1, X)) %>%
  rename(
...
samples_key_df <- read_csv(file.path(dir_data, "JVB1710-samples_rv3.csv"))
...
# save clean data
write_csv(esv_clean_data, file = file.path(dir_products, "esv_clean_data_rv.csv"))
...
```{r}
offshore_esv <- esv_clean_data %>% select(2, 6, 7, 8, 9, 10, contains('COS'), contains('AFZ'), contains('DFZ'), contains('NMS'))

nearshore_esv <- esv_clean_data %>% select(2, 6, 7, 8, 9, 10, contains('COS'), contains('SHR'), contains('LGN'), contains('FRR'))
...
```{r}
offshore_esv_sp <- esv_clean_data %>% select(2, 10, contains('COS'), contains('AFZ'), contains('DFZ'), contains('NMS'))

nearshore_esv_sp <- esv_clean_data %>% select(2, 10, contains('COS'), contains('SHR'), contains('LGN'), contains('FRR'))
