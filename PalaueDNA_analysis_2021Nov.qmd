...
if("Actinopteri" %in% tax_data$Class) {
  ...
  std_data_Act <- subset_taxa(sf, Class == "Actinopteri")
...
#std_data_Act_df <- psmelt(std_data_Act)
```
...
# data_to_rearrange <- std_data_Act@sam_data$Sample
# data_to_order_by <- std_data_Act@sam_data$Near_or_Offshore
# ordering <- order(data_to_order_by)
# std_data_Act@sam_data$Sample <- data_to_rearrange[ordering]


fish_color <- viridis(n = 90, option = "turbo")

Fig_bony_fish <- plot_bar(std_data_Act, fill = "Family") +
  theme_classic() +
  theme(axis.title.x = element_blank(),
...
plot_bar(std_data_Act, fill = "Family")
dev.off()
```
