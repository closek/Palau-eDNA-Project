...
# Original code - AM - does not build on subsequet "physeq1_mar" object because "Physeq_env" is used as the input every time
# physeq_env
...
physeq_env_mar <- physeq_env %>% 
  subset_taxa(Class %in% c("Actinopteri", "Mammalia")) %>%
  subset_taxa(!(Order %in% c("Primates"))) %>%
  subset_taxa(!(Family %in% c("Bovidae", "Canidae", "Felindae", "Hominidae", "Suidae")))
...
physeq2 <- transform_sample_counts(physeq = physeq_env_mar_pruned, 
                                   fun = standf)
...
1)  Barplot - Families - Environmental Samples (excluding Controls)
...
2) Barplot - Normalized Environmental Samples & Marine ESVs
...
3) Heatmap - Normalized Environmental Samples & Marine ESVs
...
```{r}
bar_plot_marine_mammals <- plot_bar(sfMam, fill = "Species")
